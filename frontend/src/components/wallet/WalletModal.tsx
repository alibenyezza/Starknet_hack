import { useConnect, Connector } from '@starknet-react/core';
import { IoClose } from 'react-icons/io5';
import { useEffect } from 'react';

interface WalletModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const BraavosLogo = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 160 43" fill="none" width="100" height="28">
    <path d="M21.2529 3.61754C21.3108 3.7184 21.2342 3.84306 21.1202 3.84306C18.8131 3.84306 16.9376 5.72041 16.8936 8.05325C16.1036 7.90166 15.285 7.89026 14.4763 8.03029C14.4205 5.70811 12.5498 3.84306 10.2502 3.84306C10.1362 3.84306 10.0594 3.7183 10.1174 3.61737C11.2342 1.67342 13.3095 0.367188 15.6851 0.367188C18.0608 0.367188 20.1361 1.67349 21.2529 3.61754Z" fill="url(#paint0_radial_8441_6074)"/>
    <path d="M28.086 18.0457C28.8708 18.2899 29.6102 17.581 29.3141 16.7952C27.1131 10.9552 20.5988 8.56264 15.6535 8.56264C10.6986 8.56264 4.03914 11.0325 1.98151 16.8316C1.70449 17.6123 2.44201 18.2979 3.21511 18.0544L15.0663 14.322C15.4084 14.2143 15.774 14.2137 16.1164 14.3203L28.086 18.0457Z" fill="url(#paint1_radial_8441_6074)"/>
    <path d="M1.86584 19.17L15.0763 15.0521C15.4163 14.9461 15.7793 14.9459 16.1195 15.0513L29.4129 19.172C30.523 19.5161 31.2826 20.567 31.2826 21.7587V34.174C31.2312 37.2045 28.5823 39.6335 25.6347 39.6335H20.7389C20.2549 39.6335 19.863 39.2312 19.863 38.7338V34.3808C19.863 32.6668 20.85 31.1154 22.3773 30.4288C24.4246 29.5084 26.8466 28.2634 27.3022 25.7651C27.4492 24.9588 26.9376 24.1809 26.1551 24.0211C24.1763 23.6168 21.9826 23.773 20.1275 24.6257C18.0215 25.5938 17.4706 27.2042 17.2667 29.3669L17.0195 31.6968C16.9439 32.4089 16.2944 32.9544 15.5978 32.9544C14.8771 32.9544 14.3367 32.3903 14.2586 31.6537L14.016 29.3669C13.8413 27.5145 13.5246 25.715 11.6613 24.8584C9.53582 23.8813 7.39985 23.5569 5.12759 24.0211C4.34503 24.1809 3.83344 24.9588 3.98048 25.7651C4.44 28.2852 6.84299 29.5016 8.90536 30.4288C10.4327 31.1154 11.4196 32.6668 11.4196 34.3808V38.733C11.4196 39.2304 11.0279 39.6335 10.5439 39.6335H5.64792C2.7003 39.6335 0.0514789 37.2045 0 34.174V21.7555C0 20.5653 0.75771 19.5154 1.86584 19.17Z" fill="url(#paint2_radial_8441_6074)"/>
    <path d="M41.083 34.8395V12.9591H50.9828C53.3385 12.9591 55.156 13.466 56.4353 14.4799C57.7147 15.4937 58.3544 16.88 58.3544 18.6387C58.3544 19.9422 57.9685 21.0492 57.1969 21.9595C56.4455 22.8699 55.4098 23.4803 54.0898 23.7907V23.2941C55.6332 23.5631 56.8212 24.1631 57.6538 25.0942C58.5067 26.0046 58.9331 27.1943 58.9331 28.6633C58.9331 30.6082 58.2528 32.129 56.8922 33.2256C55.552 34.3015 53.6938 34.8395 51.3179 34.8395H41.083ZM45.6217 31.2393H50.7391C51.917 31.2393 52.8206 31.0117 53.4502 30.5565C54.0797 30.1013 54.3944 29.3771 54.3944 28.384C54.3944 27.3701 54.0797 26.646 53.4502 26.2115C52.8206 25.7563 51.917 25.5287 50.7391 25.5287H45.6217V31.2393ZM45.6217 21.9595H50.1299C51.3687 21.9595 52.2825 21.7319 52.8714 21.2767C53.4603 20.8216 53.7548 20.1388 53.7548 19.2284C53.7548 18.3387 53.4603 17.6662 52.8714 17.211C52.2825 16.7558 51.3687 16.5282 50.1299 16.5282H45.6217V21.9595Z" fill="url(#paint3_radial_8441_6074)"/>
    <path d="M61.9576 34.8395V23.2631C61.9576 22.5803 61.9373 21.8871 61.8967 21.1836C61.8763 20.4802 61.8256 19.787 61.7443 19.1042H66.1612L66.679 23.6976H65.9784C66.1815 22.5803 66.5064 21.6595 66.9532 20.9354C67.4202 20.2112 67.9888 19.6732 68.659 19.3215C69.3494 18.9697 70.1313 18.7939 71.0045 18.7939C71.3903 18.7939 71.6949 18.8146 71.9183 18.8559C72.1417 18.8766 72.3651 18.9284 72.5885 19.0111L72.558 23.1389C72.1315 22.9527 71.766 22.8389 71.4614 22.7975C71.1771 22.7354 70.8116 22.7044 70.3648 22.7044C69.5119 22.7044 68.8011 22.8699 68.2325 23.201C67.6842 23.532 67.2679 24.0183 66.9836 24.6597C66.7196 25.3011 66.5876 26.077 66.5876 26.9874V34.8395H61.9576Z" fill="url(#paint4_radial_8441_6074)"/>
    <path d="M80.3636 35.1809C79.0233 35.1809 77.8455 34.8498 76.8301 34.1877C75.8148 33.5256 75.0228 32.5842 74.4542 31.3634C73.8856 30.1427 73.6013 28.6737 73.6013 26.9563C73.6013 25.2597 73.8856 23.801 74.4542 22.5803C75.0228 21.3595 75.8148 20.4284 76.8301 19.787C77.8455 19.1249 79.0233 18.7939 80.3636 18.7939C81.6024 18.7939 82.7091 19.1042 83.6839 19.725C84.6789 20.325 85.3287 21.1423 85.6334 22.1768H85.2983L85.6638 19.1042H90.0807C90.0197 19.787 89.9588 20.4802 89.8979 21.1836C89.8573 21.8664 89.837 22.5389 89.837 23.201V34.8395H85.2678L85.2374 31.8911H85.6029C85.278 32.8842 84.6281 33.6808 83.6534 34.2808C82.6786 34.8808 81.582 35.1809 80.3636 35.1809ZM81.7648 31.6428C82.8208 31.6428 83.6737 31.26 84.3235 30.4944C84.9734 29.7289 85.2983 28.5495 85.2983 26.9563C85.2983 25.3632 84.9734 24.1941 84.3235 23.4493C83.6737 22.7044 82.8208 22.332 81.7648 22.332C80.7088 22.332 79.8559 22.7044 79.2061 23.4493C78.5563 24.1941 78.2313 25.3632 78.2313 26.9563C78.2313 28.5495 78.5461 29.7289 79.1756 30.4944C79.8255 31.26 80.6885 31.6428 81.7648 31.6428Z" fill="url(#paint5_radial_8441_6074)"/>
    <path d="M99.6099 35.1809C98.2697 35.1809 97.0918 34.8498 96.0765 34.1877C95.0611 33.5256 94.2691 32.5842 93.7005 31.3634C93.1319 30.1427 92.8476 28.6737 92.8476 26.9563C92.8476 25.2597 93.1319 23.801 93.7005 22.5803C94.2691 21.3595 95.0611 20.4284 96.0765 19.787C97.0918 19.1249 98.2697 18.7939 99.6099 18.7939C100.849 18.7939 101.955 19.1042 102.93 19.725C103.925 20.325 104.575 21.1423 104.88 22.1768H104.545L104.91 19.1042H109.327C109.266 19.787 109.205 20.4802 109.144 21.1836C109.104 21.8664 109.083 22.5389 109.083 23.201V34.8395H104.514L104.484 31.8911H104.849C104.524 32.8842 103.874 33.6808 102.9 34.2808C101.925 34.8808 100.828 35.1809 99.6099 35.1809ZM101.011 31.6428C102.067 31.6428 102.92 31.26 103.57 30.4944C104.22 29.7289 104.545 28.5495 104.545 26.9563C104.545 25.3632 104.22 24.1941 103.57 23.4493C102.92 22.7044 102.067 22.332 101.011 22.332C99.9552 22.332 99.1023 22.7044 98.4524 23.4493C97.8026 24.1941 97.4777 25.3632 97.4777 26.9563C97.4777 28.5495 97.7924 29.7289 98.422 30.4944C99.0718 31.26 99.9349 31.6428 101.011 31.6428Z" fill="url(#paint6_radial_8441_6074)"/>
    <path d="M117.184 34.8395L110.514 19.1042H115.357L119.774 30.5255H118.768L123.338 19.1042H127.876L121.144 34.8395H117.184Z" fill="url(#paint7_radial_8441_6074)"/>
    <path d="M136.438 35.1809C134.793 35.1809 133.361 34.8498 132.143 34.1877C130.945 33.5049 130.01 32.5532 129.34 31.3324C128.69 30.1117 128.366 28.653 128.366 26.9563C128.366 25.2804 128.69 23.832 129.34 22.6113C130.01 21.3905 130.945 20.4491 132.143 19.787C133.361 19.1249 134.793 18.7939 136.438 18.7939C138.083 18.7939 139.514 19.1249 140.733 19.787C141.951 20.4491 142.885 21.3905 143.535 22.6113C144.205 23.832 144.54 25.2804 144.54 26.9563C144.54 28.653 144.205 30.1117 143.535 31.3324C142.885 32.5532 141.951 33.5049 140.733 34.1877C139.514 34.8498 138.083 35.1809 136.438 35.1809ZM136.438 31.6428C137.494 31.6428 138.326 31.26 138.936 30.4944C139.565 29.7289 139.88 28.5495 139.88 26.9563C139.88 25.3632 139.565 24.1941 138.936 23.4493C138.326 22.7044 137.494 22.332 136.438 22.332C135.382 22.332 134.539 22.7044 133.909 23.4493C133.3 24.1941 132.996 25.3632 132.996 26.9563C132.996 28.5495 133.3 29.7289 133.909 30.4944C134.539 31.26 135.382 31.6428 136.438 31.6428Z" fill="url(#paint8_radial_8441_6074)"/>
    <path d="M153.207 35.1809C151.826 35.1809 150.567 35.0257 149.43 34.7153C148.293 34.405 147.328 33.9705 146.536 33.4118L147.724 30.3082C148.516 30.8048 149.389 31.1979 150.344 31.4876C151.318 31.7773 152.283 31.9221 153.237 31.9221C154.131 31.9221 154.791 31.7773 155.217 31.4876C155.644 31.1979 155.857 30.8048 155.857 30.3082C155.857 29.8944 155.715 29.5737 155.431 29.3461C155.167 29.1185 154.75 28.953 154.182 28.8495L151.044 28.2598C149.724 27.9909 148.719 27.4943 148.029 26.7701C147.338 26.0459 146.993 25.1045 146.993 23.9458C146.993 22.932 147.257 22.0423 147.785 21.2767C148.333 20.4905 149.115 19.8801 150.13 19.4456C151.146 19.0111 152.334 18.7939 153.694 18.7939C154.852 18.7939 155.948 18.9387 156.984 19.2284C158.02 19.518 158.903 19.9629 159.634 20.5629L158.385 23.5734C157.756 23.0975 157.025 22.7147 156.192 22.4251C155.38 22.1354 154.598 21.9906 153.847 21.9906C152.872 21.9906 152.171 22.1561 151.745 22.4872C151.318 22.7975 151.105 23.201 151.105 23.6976C151.105 24.07 151.227 24.3803 151.471 24.6286C151.735 24.8769 152.131 25.0631 152.659 25.1873L155.827 25.777C157.187 26.0253 158.223 26.4908 158.934 27.1736C159.644 27.8564 160 28.7978 160 29.9979C160 31.0945 159.715 32.0255 159.147 32.7911C158.578 33.5567 157.776 34.1463 156.74 34.5601C155.725 34.974 154.547 35.1809 153.207 35.1809Z" fill="url(#paint9_radial_8441_6074)"/>
    <defs>
      <radialGradient id="paint0_radial_8441_6074" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(15.6413 20.0003) rotate(90) scale(19.6332 15.6413)"><stop offset="0.47" stopColor="#587EFF"/><stop offset="1" stopColor="#5478F4"/></radialGradient>
      <radialGradient id="paint1_radial_8441_6074" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(15.6413 20.0003) rotate(90) scale(19.6332 15.6413)"><stop offset="0.47" stopColor="#587EFF"/><stop offset="1" stopColor="#5478F4"/></radialGradient>
      <radialGradient id="paint2_radial_8441_6074" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(15.6413 20.0003) rotate(90) scale(19.6332 15.6413)"><stop offset="0.47" stopColor="#587EFF"/><stop offset="1" stopColor="#5478F4"/></radialGradient>
      <radialGradient id="paint3_radial_8441_6074" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(100.541 24.07) rotate(90) scale(11.1109 59.4584)"><stop offset="0.47" stopColor="#587EFF"/><stop offset="1" stopColor="#5478F4"/></radialGradient>
      <radialGradient id="paint4_radial_8441_6074" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(100.541 24.07) rotate(90) scale(11.1109 59.4584)"><stop offset="0.47" stopColor="#587EFF"/><stop offset="1" stopColor="#5478F4"/></radialGradient>
      <radialGradient id="paint5_radial_8441_6074" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(100.541 24.07) rotate(90) scale(11.1109 59.4584)"><stop offset="0.47" stopColor="#587EFF"/><stop offset="1" stopColor="#5478F4"/></radialGradient>
      <radialGradient id="paint6_radial_8441_6074" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(100.541 24.07) rotate(90) scale(11.1109 59.4584)"><stop offset="0.47" stopColor="#587EFF"/><stop offset="1" stopColor="#5478F4"/></radialGradient>
      <radialGradient id="paint7_radial_8441_6074" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(100.541 24.07) rotate(90) scale(11.1109 59.4584)"><stop offset="0.47" stopColor="#587EFF"/><stop offset="1" stopColor="#5478F4"/></radialGradient>
      <radialGradient id="paint8_radial_8441_6074" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(100.541 24.07) rotate(90) scale(11.1109 59.4584)"><stop offset="0.47" stopColor="#587EFF"/><stop offset="1" stopColor="#5478F4"/></radialGradient>
      <radialGradient id="paint9_radial_8441_6074" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(100.541 24.07) rotate(90) scale(11.1109 59.4584)"><stop offset="0.47" stopColor="#587EFF"/><stop offset="1" stopColor="#5478F4"/></radialGradient>
    </defs>
  </svg>
);

const READY_LOGO_URL = 'https://cdn.prod.website-files.com/680f9220d4668b0e40862420/681b4c1ea23569cd7aef6add_4dd6e26543621bf8b5a31d5a93f83136_ready-logo-linear.svg';

const walletInfo: Record<string, { name: string; icon: React.ReactNode; description: string }> = {
  argentX: {
    name: 'Ready',
    icon: <img src={READY_LOGO_URL} alt="Ready" style={{ width: 28, height: 28, objectFit: 'contain' }} />,
    description: 'Formerly Argent X'
  },
  braavos: {
    name: 'Braavos',
    icon: <BraavosLogo />,
    description: 'Secure Starknet wallet'
  },
  argentMobile: {
    name: 'Argent Mobile',
    icon: <img src={READY_LOGO_URL} alt="Ready Mobile" style={{ width: 28, height: 28, objectFit: 'contain' }} />,
    description: 'Mobile Starknet wallet'
  },
  argentWebWallet: {
    name: 'Argent Web',
    icon: <img src={READY_LOGO_URL} alt="Ready Web" style={{ width: 28, height: 28, objectFit: 'contain' }} />,
    description: 'Web-based wallet'
  }
};

export function WalletModal({ isOpen, onClose }: WalletModalProps) {
  const { connect, connectors } = useConnect();

  useEffect(() => {
    const handleEscape = (e: KeyboardEvent) => {
      if (e.key === 'Escape') onClose();
    };
    if (isOpen) {
      document.addEventListener('keydown', handleEscape);
      document.body.style.overflow = 'hidden';
    }
    return () => {
      document.removeEventListener('keydown', handleEscape);
      document.body.style.overflow = 'unset';
    };
  }, [isOpen, onClose]);

  if (!isOpen) return null;

  const handleConnect = async (connector: Connector) => {
    try {
      await connect({ connector });
      onClose();
    } catch (error) {
      console.error('Failed to connect:', error);
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ fontFamily: 'var(--font-ui)' }}>
      {/* Backdrop */}
      <div
        className="absolute inset-0 bg-black/80 backdrop-blur-sm"
        onClick={onClose}
      />

      {/* Modal */}
      <div
        className="relative rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl animate-scale-in"
        style={{
          background: 'rgba(5, 5, 18, 0.97)',
          border: '1px solid rgba(39, 39, 87, 0.5)',
        }}
      >
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold gradient-text">Connect Wallet</h2>
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-white transition-colors p-2 rounded-lg"
            style={{ background: 'rgba(39, 39, 87, 0.2)' }}
            aria-label="Close modal"
          >
            <IoClose className="w-6 h-6" />
          </button>
        </div>

        {/* Description */}
        <p className="text-gray-400 text-sm mb-6">
          Choose your preferred Starknet wallet to connect
        </p>

        {/* Wallet List */}
        <div className="space-y-3">
          {connectors.map((connector) => {
            const info = walletInfo[connector.id] || {
              name: connector.name || connector.id,
              icon: null,
              description: 'Starknet wallet'
            };

            return (
              <button
                key={connector.id}
                onClick={() => handleConnect(connector)}
                className="w-full flex items-center gap-4 p-4 rounded-xl transition-all duration-200 group"
                style={{
                  background: 'rgba(39, 39, 87, 0.15)',
                  border: '1px solid rgba(39, 39, 87, 0.4)',
                }}
                onMouseEnter={e => {
                  (e.currentTarget as HTMLButtonElement).style.background = 'rgba(39, 39, 87, 0.35)';
                  (e.currentTarget as HTMLButtonElement).style.borderColor = 'rgba(68, 68, 194, 0.6)';
                }}
                onMouseLeave={e => {
                  (e.currentTarget as HTMLButtonElement).style.background = 'rgba(39, 39, 87, 0.15)';
                  (e.currentTarget as HTMLButtonElement).style.borderColor = 'rgba(39, 39, 87, 0.4)';
                }}
              >
                <div style={{ width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {info.icon}
                </div>
                <div className="flex-1 text-left">
                  <div className="font-semibold text-white">
                    {info.name}
                  </div>
                  <div className="text-sm text-gray-400">
                    {info.description}
                  </div>
                </div>
                <div
                  className="transition-colors"
                  style={{ color: 'rgba(68, 68, 194, 0.6)' }}
                >
                  →
                </div>
              </button>
            );
          })}
        </div>

        {/* No wallets found message */}
        {connectors.length === 0 && (
          <div className="text-center py-8 text-gray-400">
            <p className="mb-4">No Starknet wallets detected</p>
            <p className="text-sm">
              Please install{' '}
              <a
                href="https://www.getready.so/"
                target="_blank"
                rel="noopener noreferrer"
                className="underline hover:text-white"
                style={{ color: '#8080ff' }}
              >
                Ready
              </a>
              {' '}or{' '}
              <a
                href="https://braavos.app/"
                target="_blank"
                rel="noopener noreferrer"
                className="underline hover:text-white"
                style={{ color: '#8080ff' }}
              >
                Braavos
              </a>
            </p>
          </div>
        )}

        {/* Footer */}
        <div className="mt-6 pt-6" style={{ borderTop: '1px solid rgba(39, 39, 87, 0.3)' }}>
          <p className="text-xs text-gray-500 text-center">
            By connecting, you agree to our Terms of Service
          </p>
        </div>
      </div>
    </div>
  );
}
