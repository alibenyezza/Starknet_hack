...
export function useVaultManager() {
      ...
      try {
        ...
        if (address) {
          ...
          setStakerStats({
            ...
            pendingRewards: fromWei(pending, 18),
            rewardRate,
          });
        }
      } catch (err) {
  ...
  // ── Deposit wBTC + stake in one multicall ────────────────────────────
  const depositAndStake = useCallback(async (
    ...
    try {
      ...
      if (!isDeployed(CONTRACTS.VAULT_MANAGER)) return { success: false, error: 'VaultManager not deployed' };
      if (!isDeployed(CONTRACTS.STAKER))        return { success: false, error: 'Staker contract not deployed' };
      if (!isDeployed(CONTRACTS.LT_TOKEN))      return { success: false, error: 'LT Token not deployed' };
